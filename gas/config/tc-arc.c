...
arc_optimize_expr (expressionS *l, operatorT op, expressionS *r)
{
  ...
  if (op == O_bit_and
      ...
      /* FIXME: want better test if symbol can be evaluated.  */
      && (1||S_GET_VALUE (l->X_add_symbol) || l->X_add_number))
    {
      offsetT v;

      if (!resolve_expression (l))
	return 0;
      v = S_GET_VALUE (l->X_add_symbol) + l->X_add_number;

      fprintf(stderr,"r->X_add_number: %ld\n",r->X_add_number);
      fprintf(stderr,"v: %ld\n",v);
      l->X_add_number = (v & r->X_add_number);
      fprintf(stderr,"l->X_add_number: %ld\n",l->X_add_number);
      l->X_op = O_constant;
      l->X_add_symbol = 0;
      return 1;
    }
  ...
  if (op == O_bit_and
      ...
      && (1||S_GET_VALUE (l->X_add_symbol) || l->X_add_number))
    {
      ...
      fprintf(stderr,"v: %ld\n",v);
      l->X_add_number -= (v & ~r->X_add_number);
      fprintf(stderr,"new l->X_add_number: %ld\n",l->X_add_number);
      return 1;
    }
  return 0;
}
