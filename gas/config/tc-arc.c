...
static void arc_extinsn (int ignore ATTRIBUTE_UNUSED)
{
  ...
  if (hash_find (arc_opcode_hash, einsn.name))
    as_fatal ("Pseudocode already used %s", einsn.name);
  ...
  if ((einsn.major > mophigh) || (einsn.major < moplow))
    as_fatal ("major opcode not in range [0x%02x - 0x%02x]", moplow, mophigh);

  if ((einsn.minor > 0x3f) && (einsn.major != 0x0a)
      && (einsn.major != 5) && (einsn.major != 9))
    as_fatal ("minor opcode not in range [0x00 - 0x3f]");

  switch (einsn.syntax & (ARC_SYNTAX_3OP | ARC_SYNTAX_2OP))
    {
    case ARC_SYNTAX_3OP:
      if (einsn.modsyn & ARC_OP1_IMM_IMPLIED)
	as_fatal ("Improper use of OP1_IMM_IMPLIED");
      break;
    case ARC_SYNTAX_2OP:
      if (einsn.modsyn & ARC_OP1_MUST_BE_IMM)
	as_fatal ("Improper use of OP1_MUST_BE_IMM");
      break;
    default:
      break;
    }
  ...
  if (arc_ext_opcodes == NULL)
	...
	as_fatal ("Couldn't generate extension instruction opcodes");
    }
  else if (errmsg)
    as_warn ("%s", errmsg);

  /* Insert the extension instruction.  */
  retval = hash_insert (arc_opcode_hash, arc_ext_opcodes->name,
			(void *) arc_ext_opcodes);
  if (retval)
    as_fatal (_("internal error: can't hash opcode '%s': %s"),
	      arc_ext_opcodes->name, retval);
