...
/* Implementation of linux_target_ops method "breakpoint_at".
   ...
   The breakpoint instruction is TRAP_S 1, network function ntohs can be used
   to convert its little-endian form (0x3e78) to the host representation, which
   may be little-endian or big-endian (network representation is defined to be
   little-endian).  */
...
/* PTRACE_GETREGSET/NT_PRSTATUS and PTRACE_SETREGSET/NT_PRSTATUS work with
   ...
   linux/arch/arc/include/uapi/asm/ptrace.h header.  This code supports ARC
   Linux ABI v3 and v4.  */

/* Populate a ptrace NT_PRSTATUS regset from a regcache.

   This appears to be a unique approach to populating the buffer, but being
   name, rather than offset based, it is robust to future API changes, as there
   is no need to create a regmap of registers in the user_regs_struct.  */
...
arc_fill_gregset (struct regcache *regcache, void *buf)
{
  ...
  /* Currently ARC Linux ptrace doesn't allow writes to status32 because some
     of it's bits are kernel mode-only and shoudn't be writable from
     user-space.  Writing status32 from debugger could be useful, though, so
     ability to write non-priviliged bits will be added to kernel sooner or
     later.  */
