...
arc_insn_get_branch_target (const struct arc_instruction &insn)
{
  ...
  /* JLI and EI depend on optional AUX registers.  Not supported right now.  */
    ...
    {
      gdb_printf (gdb_stderr, "JLI_S instruction is not supported by the GDB.");
      return 0;
    }
		     ...
		     CORE_ADDR struct_addr)
{
  ...
  if (nargs > 0)
      ...
      for (int i = 0; i < nargs; i++)
	{
      ...
      data = memory_image;
      const int reg_size = arc64_isa_reg_size (gdbarch);
  ...
  if (len <= reg_size)
    {
      ...
      arc_debug_printf ("returning 0x%s", phex (val, reg_size));
    }
  else if (len <= reg_size * 2)
    {
      ULONGEST low, high;
      ...
      store_unsigned_integer (valbuf, reg_size,
  ...
  if (len <= reg_size)
    {
      ...
      arc_debug_printf ("storing 0x%s", phex (val, reg_size));
    }
  else if (len <= reg_size * 2)
    {
      ULONGEST low, high;
      ...
      low = extract_unsigned_integer (valbuf, reg_size,
				      gdbarch_byte_order (gdbarch));
      high = extract_unsigned_integer (valbuf + reg_size,
				       (int) len - reg_size,
				       gdbarch_byte_order (gdbarch));
...
arc_get_longjmp_target (frame_info_ptr frame, CORE_ADDR *pc)
{
  ...
  const int reg_size = arc64_isa_reg_size (gdbarch);
  int pc_offset = tdep->jb_pc * reg_size;
  gdb_byte buf[8];
  CORE_ADDR jb_addr = get_frame_register_unsigned (frame, ARC_FIRST_ARG_REGNUM);

  if (target_read_memory (jb_addr + pc_offset, buf, reg_size))
    return 0; /* Failed to read from memory.  */

  *pc = extract_unsigned_integer (buf, reg_size,
				  gdbarch_byte_order (gdbarch));
  return 1;
}
	  ...
	  if (insn.operands[0].kind == ARC_OPERAND_KIND_REG)
	    {
	      ...
	      stack->store (addr, reg_size, store_value1);
	      stack->store (pv_add_constant (addr, reg_size),
			    reg_size, store_value2);
	    }
	  else
	    {
	      pv_t store_value
		= pv_constant (arc_insn_get_operand_value (insn, 0));
	      stack->store (addr, reg_size * 2, store_value);
	    }
	}
...
int
arc64_isa_reg_size (struct gdbarch *gdbarch)
{
  const arc64_gdbarch_tdep *tdep
    = gdbarch_tdep<arc64_gdbarch_tdep> (gdbarch);
  return tdep->features.reg_size;
}
...
arc64_gdbarch_init (struct gdbarch_info info, struct gdbarch_list *arches)
{
  ...
  /* Reconstructing FEATURES from the initialized TDESC.  */
  const struct tdesc_feature *feat_aux
    = tdesc_find_feature (tdesc, ARC_AUX_FEATURE_NAME);
  const int reg_size = tdesc_register_bitsize (feat_aux, "pc") / 8;
  const arc_arch_features features(reg_size, ARC_ISA_ARCV3);

  /* Check if there is any ARCH that matches the current FEATURES.  */
  for (arches = gdbarch_list_lookup_by_info (arches, &info);
       arches != NULL;
       arches = gdbarch_list_lookup_by_info (arches->next, &info))
    {
      const arc64_gdbarch_tdep *other_tdep
	= gdbarch_tdep<arc64_gdbarch_tdep> (arches->gdbarch);

      if (other_tdep->features == features)
	break;
    }

  /* If a match was found, reuse it.  */
  if (arches != NULL)
    return arches->gdbarch;

  /* Nothing found.  Create a new architecture from the available data.  */
  //arc64_gdbarch_tdep *tdep = new (arc64_gdbarch_tdep) (features);
  //struct gdbarch *gdbarch = gdbarch_alloc (&info, tdep);
  gdbarch *gdbarch
    = gdbarch_alloc (&info,
		     gdbarch_tdep_up (new arc64_gdbarch_tdep (features)));
  arc64_gdbarch_tdep *tdep = gdbarch_tdep<arc64_gdbarch_tdep> (gdbarch);

  /* Data types.  */
  set_gdbarch_short_bit (gdbarch, 16);
  set_gdbarch_int_bit (gdbarch, 32);
  set_gdbarch_long_bit (gdbarch, arc64_isa_reg_size (gdbarch) * 8);
  ...
  set_gdbarch_ptr_bit (gdbarch, arc64_isa_reg_size (gdbarch) * 8);
  set_gdbarch_addr_bit (gdbarch, arc64_isa_reg_size (gdbarch) * 8);
...
dump_arc_instruction_command (const char *args, int from_tty)
{
  ...
  struct arc_instruction insn;
  struct gdb_non_printing_memory_disassembler dis (current_inferior ()->arch ());
