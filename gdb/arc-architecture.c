...
arc_get_architecture (ARC_RegisterNumber identity_regnum)
{
...
void
arc_update_architecture (ARC_RegisterNumber identity_regnum)
{
...
{
  ARC_ProcessorVersion target =
    gdbarch_tdep (gdbarch)->processor_variant_info->processor_version;
...
//  printf_filtered("program = %s\n", arc_version_image(program));

  {
    ...
    if (params == 2)
      ...
      warning (message, arc_version_image (target), objfile_bfd->filename,
	       arc_version_image (program), arc_version_image (auxregs));
  }
